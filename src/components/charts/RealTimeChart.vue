...
const currentYear = ref(
	parseInt(mapStore.currentIndex) + 2022 ? 2022 : props.filterYear
);

function scaleY(value) {
	const plotHeigh = svgHeight - 2 * yAxisHeight;
	const labelValues = yLabels.value.labels.map((label) =>
	...
	for (let i = 0; i < labelValues.length - 1; i++) {
		if (value >= labelValues[i] && value <= labelValues[i + 1]) {
			...
			return (
				plotHeigh -
				(i + 1) * intervalHeigh +
				relativePosition * intervalHeigh +
				yAxisHeight
			);
		}
	}
...
function scaleX(value) {
	...
	const scaledValue =
		...
		((value - minValue) / (maxValue - minValue)) *
			(svgWidth - xAxisWidth * 2);
	return scaledValue > svgWidth - xAxisWidth
		? svgWidth - xAxisWidth + 5
		: scaledValue;
}
			...
			dataPoints.value
				.filter((point) => {
					return (
						...
						parseInt(mapStore.currentIndex) + 2022 >= point.year
					);
				})
				.map((point, index) => ({
					index: index,
...
const isShowTooltip = (point) => {
	if (point.year === parseInt(mapStore.currentIndex) + 2022) return true;
...
const isShowConnectLine = (currentDataPoint, currentDataPointIndex) => {
	if (currentDataPoints.value[currentDataPointIndex + 1]) {
		return (
			...
			currentDataPoints.value[currentDataPointIndex + 1]?.category
		);
	}
	return false;
};
...
<template>
	<div
		v-if="activeChart === 'RealTimeChart'"
		:key="updateTrigger"
		ref="containerRef"
	>
		<svg :width="svgWidth" :height="svgHeight">
			...
			<g v-show="currentDataPoints.length > 0">
				...
				<g
					...
					:id="`vzb-bc-bubble-${currentDataPoint.category}-${
						activeCountries.includes(currentDataPoint.category)
